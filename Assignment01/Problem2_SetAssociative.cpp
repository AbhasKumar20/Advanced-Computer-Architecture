...
bool SearchL3(vector< vector<Cache> > &L3)
{
	for (int set = 0; set < 2048; ++set)
	{
		for (int line = 0; line < 16; ++line)
		{
			if(L3[set][line].present==0)
				return false;
		}
	}
...
void evictL3(vector< vector<Cache> > &L3 , vector< vector<Cache> > &L2,int L3_SET,unsigned long long L3_TAG, unsigned long long min, unsigned long long* priority )
{													// Replace in L3 Cache 
	int index=0;
	...
	unsigned long long L3_ReTAG = L3[L3_SET][index].tag;				// Place in L3 Cache

	unsigned long long L2_SET = SET_NUM((L3_ReTAG | L3_SET),10); 		//Extract L2 Set number from L3 tag and Set number
	unsigned long long L2_TAG = SHIFT_RIGHT((L3_ReTAG | L3_SET),10);    //Extract L2 Tag from L3 Tag and Set number
...
int main(int argc, char** argv)
{
	string applications[6] = { "h264ref.log_l1misstrace", "hmmer.log_l1misstrace", "bzip2.log_l1misstrace", 
									"gcc.log_l1misstrace","gromacs.log_l1misstrace","sphinx3.log_l1misstrace"};
	int numtraces[6]={1,1,2,2,1,2};
	cout<< "-------------------------------------------------------------------------" << "\n";
	...
	for(int i=0;i<6;++i)
	{
		...
		FILE * fp;
		char iord,type;
		unsigned long long addr,L2_TAG,L3_TAG,priority=0,min,L3_ReTAG, L1_misses=0,L3_conflictMiss=0,L3_capacityMiss=0;
		unsigned pc;
		     	...
		     	else
		     	{
					...
					for(int line=0;line<16;++line)
					{
		     		...
		     		if(L3_HIT==false)
		     		{
		     			...
		     			for (int i = 0; i < 16; ++i)
		     			{
		     				if(L3[L3_SET][i].present==0)     //Try to place in L3 Cache in L3_SET
		     				{
		     					empty=true;
		     			...
		     			if(empty==false)						// When can't be placed in L3_SET
		     			{
	                        L3_conflictMiss++;
	                       
	                        if(SearchL3(L3))                     // When L3 Cache is full
	                        	L3_capacityMiss++;                     
	                        											//If can't be placed in L3_SET
		     				evictL3(L3,L2,L3_SET,L3_TAG,L3[L3_SET][0].time,&priority); //Replace in L3 also evict in L2 
		...
		cout << "L3_conflictMisses:" << L3_conflictMiss << "\n";
		cout << "L3_capacityMiss:" << L3_capacityMiss << "\n";
